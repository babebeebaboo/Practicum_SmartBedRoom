...
var LightController = {
  name: "Light0", //name of accessory
  pincode: "584-94-590",
  username: "FB:3D:BA:5B:1B:1A", // MAC like address used by HomeKit to differentiate accessories.
  manufacturer: "Practicum", //manufacturer (optional)
  model: "v1.0", //model (optional)
  serialNumber: "A75S345KGA", //serial number (optional)
  ...
  setPower: function (status) { //set power of accessory
    ...
    if (status) {
      ...
      request('http://localhost:30000/light0?set=on', function (error, response, body) {
    ...
    //off
      ...
      var request = require('request');
      request('http://localhost:30000/light0?set=off', function (error, response, body) {
        console.log('error:', error); // Print the error if one occurred
        console.log('statusCode:', response && response.statusCode); // Print the response status code if a response was received
        console.log('body:', body); // Print the HTML for the Google homepage.
      });
  ...
  getLight: function () {
    var request = require('request');
    request('http://localhost:30000/light0', function (error, response, body) {
      console.log('error:', error); // Print the error if one occurred
      console.log('statusCode:', response && response.statusCode); // Print the response status code if a response was received
      console.log('body:', body); // Print the HTML for the Google homepage.
      this.power = parseInt(body);
    });
  }
}
