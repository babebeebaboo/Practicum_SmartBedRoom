...
var FAKE_LOCK = {
  ...
  lock: function () {
    ...
    request('http://localhost:30000/lock?set=on', function (error, response, body) {
  ...
  unlock: function () {
    ...
    var request = require('request');
    request('http://localhost:30000/lock?set=off', function (error, response, body) {
      console.log('error:', error); // Print the error if one occurred
      console.log('statusCode:', response && response.statusCode); // Print the response status code if a response was received
      console.log('body:', body); // Print the HTML for the Google homepage.
    });
  ...
  identify: function () {
    ...
    var request = require('request');
    request('http://localhost:30000/lock', function (error, response, body) {
      console.log('error:', error); // Print the error if one occurred
      console.log('statusCode:', response && response.statusCode); // Print the response status code if a response was received
      console.log('body:', body); // Print the HTML for the Google homepage.
    });
